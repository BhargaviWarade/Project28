...
function handlePlayerArrowCollision() {
  for (var i = 0; i < playerArrows.length; i++) {
    ...
    if (playerArcherLife <= 0){
      ...
      Matter.Body.setStatice(playerArcher.body, false);
      Matter.Body.setStatice(player.body,false);
      playerArcherLife -=1;
...
function handleComputerArrowCollision() {
  for (var i = 0; i < computerArrows.length; i++) {
    ...
    }
    {
        ...
        Matter.Body.setStatice(computerArcher.body, false);
        Matter.Body.setStatice(computer.body,false);
        MacomputerArcherLife -=1;
      computer.reduceLife(computerArcherLife);
    tter.Body.setPosition(computer.body, {
          x: width - 100,
...
/*function showArrows(index , arrows){
  ...
  if (
    ...
    arrows[index].body.position.y > width 
  )  {
